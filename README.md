...
after it is a **per-device** secret, exactly as before — freely chosen per
device, not required to be unique, and not generated by this server:
...
- **Company slug**: identifies which company a not-yet-registered device's
  ...
  It plays no role once a device is actually registered — SN + per-device
  secret alone decide trust for a claimed device, regardless of which slug
  (if any) the request arrived with.
- **Per-device secret**: **pick it yourself** (or use "Generate" in the
  device's edit drawer) and put the full URL — company slug **and**
  secret — into the device's Cloud Server Setting *before* or *after*
  registering it; either order works. If it pings in before you've
  registered it, the secret is captured automatically and carried into the
...
- A device with no secret configured stays fully open on its company's
  plain `<host>:<port>/<company-slug>/` URL — this is the default for
  every new/legacy device and isn't a forced migration; secure devices one
